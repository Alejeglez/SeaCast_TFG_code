...
import os
from src.seacast_tools.noise_generator import  NoiseGenerator
...
class WeatherDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.noise is not None:
            noise_generator_ob = NoiseGenerator(self.noise)
            np_sample[:2] = noise_generator_ob.apply_noise(np_sample[:2])

        try:
            full_sample = torch.tensor(
                np_sample, dtype=torch.float32
