...
from typing import Any, Callable, Hashable, Iterable, Mapping, Optional, Union, List, Tuple
import numpy as np
import os
...
def _load_numpy_file(source: Union[str, List[str]], start_date: Optional[np.datetime64] = None, end_date: Optional[np.datetime64] = None) -> List:
...
def _apply_mask_to_entry(data_entry: np.ndarray, mask: np.ndarray) -> np.ndarray:
    ...
    data_grid = np.full(mask.shape, np.nan, dtype=np.float16)
...
class NpLoaders(base.DataLoader):
    ...
    def __init__(
        ...
        dates: Optional[Tuple[np.datetime64, np.datetime64]] = None
    ):
        super().__init__()
    ...
    def _load_and_process_data(self):
        ...
        _data_entries, self._init_dates = _load_numpy_file(self.path, self.start_date, self.end_date)

        print(f"Loaded {len(_data_entries)} data entries from {self.path}")

        processed_data = [_apply_mask(_data_entries, self.mask)]

        self.all_data = [np.asarray(d, dtype=np.float16) for d in processed_data]
...
class TargetsFromNumpy(NpLoaders):
    ...
    def __init__(self, path: str, variables: Iterable[str], extra_files: Optional[Iterable[str]] = None, extra_variables: Optional[Iterable[str]] = None, dates: Optional[Tuple[np.datetime64, np.datetime64]] = None):
        super().__init__(path, variables, extra_files, extra_variables, dates)
